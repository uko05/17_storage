...
- **タグ付けは右クリックメニューに直接埋め込み方式**(2026-09-09、
  「タグを選択」という一枚挟むポップアップ方式から変更)。専用モーダルは
  廃止し、`buildTagMenuItems(imageIds)`が候補タグ(+「＋ 新しいタグを追加」)を
  そのまま右クリックメニューの項目として返す。1枚右クリック時は
  `拡大表示/ダウンロード/共有用URLをコピー/<タグ一覧>/削除する`の並びで、
  タグ一覧は「共有用URLをコピー」の直下に挿入している。
  - 1枚編集時: 各タグの行に既存タグなら`✓ `を付けて表示し、クリックで
    その場でON/OFFする(`toggleTagOnImage()`。1回のクリックで即確定、
    確認・適用ボタンは無い)。
  - 複数選択時(`selectedImageIds.size > 0`): 右クリックメニューはタグ一覧
    だけになり(拡大表示等は出さない)、クリックしたタグを選択中の全画像に
    "追加"する(`addTagToImages()`、**追加のみ**で既存タグの削除はしない
    — 画像ごとに元のタグが違うため)。選択状態はタグ適用後もクリアしない
    (続けて別のタグも追加できるように)。
