...
- タグ付けは**アップロード後**、画像の右クリックから行う(アップロード時には
  タグを聞かない)。
- 絞り込み行の左に「選択」チェックボックスを追加(`#select-mode-toggle`)。
  ONにすると各画像に選択用チェックボックスが出る(`.storage-card-select`、
  CSSは`#gallery-grid.select-mode`配下でのみ表示)。
- 右クリック時、1枚以上チェックが付いていれば「タグを選択(N件に追加)」だけの
  メニューになり、選択中の全画像にまとめてタグを追加する(**追加のみ**、
  既存タグの削除はしない。画像ごとに元のタグが違うため、混在状態を安全に
  UI表示するのが煩雑になるのを避けた設計判断)。
  チェックが1つも無ければ、右クリックした1枚だけを対象にした通常の
  タグ編集(チェックを外す/チップの×で既存タグも削除できる)になる。
- タグ選択ポップアップ(`#tag-picker`)はプリセットのチェックボックス＋
  独自タグ入力欄(Enterでも追加可)の組み合わせ。プリセットに無い文字列は
  自由に追加できる(「右クリックでユーザー独自のタグを作成もできる」という
  要望通り)。
- **重要**: `#tag-picker`は元は`hidden`クラス付きで置いていたが、
  `.storage-tag-picker.open{display:flex}`と`.hidden{display:none!important}`
  が同時に付くと`!important`の方が勝って永久に表示されないバグを踏んだ。
  ライトボックス(`#storage-lightbox`)と同じく、`.storage-tag-picker`自体に
  `display:none`のデフォルトを持たせ、`hidden`汎用クラスは使わない方式に
  している。同種のポップアップを追加する時はこのパターンを踏襲すること。
