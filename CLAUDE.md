...
- **整理方法**: フォルダ/アルバムを強制しない。アップロード日時順の一覧が基本で、
  そこに任意のタグを軽く乗せる方式(Google Photos/Immichが支持されている
  理由の分析から)。お気に入り(★)機能は2026-09-10に廃止した(UIから使われて
  おらず、タグでの整理と役割が重複していたため)。
...
## 元画像のまま保存する機能 → 実装・撤回(2026-09-08実装 → 2026-09-10撤回)
一時期、08_UPointで100UPと交換すると原寸のまま保存できる機能
(`sitePerks.storage17.originalUpload`)があったが、**誰も交換していない
うちに撤回した**(容量リスクを避けるため)。撤回した範囲:
- `17_storage`: アップロード画面のチェックボックス、`uploadOneFile()`の
  `isOriginal`分岐、`DAILY_ORIGINAL_UPLOAD_LIMIT`を全削除。今は常に
  `view.webp`(長辺1920px)で保存する。
- `24_AccountCenter/storage.rules`: `screenshotStorage/`の`original.*`許可を削除。
- `08_UPoint/script.js`: `storage17_original_upload`アイテムとi18nテキストを削除。
  同じsiteKey内の`storage17_extra_daily_upload`(1日のアップロード上限+1枚、
  50UP)は**別機能なので存続**。
再実装する場合は、この一連のコミット(2026-09-10)を参考に、容量試算を
先に詰めてから進めること。
...
- タグ付けは**アップロード後**、画像の右クリックから行う(アップロード時には
  タグを聞かない)。
- 絞り込み行に「複数選択」チェックボックス(`#select-mode-toggle`)と
  「画像削除」ボタン(`#bulk-delete-btn`)を並べて配置。選択用チェックボックス
  自体(`.storage-card-select`)は**常時表示**(2026-09-10、「複数選択」ON時
  だけ表示する仕様から変更。「複数選択」チェックが無くてもいつでも画像を
  選べるように)。「複数選択」チェックは今も意味があり、ONの間はカードの
  どこをクリックしてもチェックがトグルする(`selectModeEnabled`の時は
  カードのclickでライトボックスを開かずチェックをトグルするよう分岐)。
  OFFの間は左上の小さいチェックボックス自体をタップ/クリックしてしか
  選べない(通常のクリックは今まで通りライトボックスを開く)。
  「画像削除」ボタンは選択中の画像があれば`deleteImages()`で一括削除する
  (`bulkDeleteBtn`は「複数選択」がONかどうかに関係なく機能する)。
  この節の冒頭にあった長い説明文(`#select-mode-hint`)は「直感的に使える
  UIを目指す」方針のため2026-09-10に削除した(操作から機能を理解できる
  よう、UI側の工夫で説明不要にする方向)。
...
- アップロード: **1日30枚/人**(`myDailyUploadLimit()`が
  `sitePerks.storage17.extraDailyUploads`を加算する。08_UPointで50UP/回・
  最大10回まで交換でき、+1枚ずつ永続的に底上げできる。合計で最大+10枚まで)

**根拠(仮の試算、2026-09-08時点)**: 1日あたりの利用者数を仮に30人と想定。
画像1枚あたりview+thumbで概算350KBと仮定すると、全員が毎日上限までフルに
使った最悪ケースでも月間のストレージ増加は概算10GB程度(Firebase Storage
課金は$0.026/GB/月程度なので、月100円未満で収まる計算)。実際の利用者数が
想定と大きくズレる場合は、この30人という前提ごと見直して上限を調整すること。
(2026-09-09に実データを計測したところ実際の平均は1枚181KBで、この仮定より
軽かった。「元画像のまま保存」機能は2026-09-10に撤回済みなので、この試算には
もう含めていない。)
